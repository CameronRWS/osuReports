...
function initialize() {
    ...
    db.all("SELECT osuUsername, twitterUsername FROM playersTable", (err, rows) => {
        for (var i = 0; i < rows.length; i++) {
            globalInstances.playerObjects.push(new playerObject(rows[i].osuUsername, rows[i].twitterUsername));
        }
        globalInstances.logMessage("From initialize(): Starting to loop through players...");
    });


    setTimeout(loopThroughPlayers, 5000);
}
...
function getSessionInfoForConsole() {
    ...
    output += ("\nDate: " + (currentTime.getMonth() + 1) + "/" + currentTime.getDate() + "/" + currentTime.getFullYear() + " | Time: " + currentTime.getHours() + ":" + currentTime.getMinutes() + ":" + currentTime.getSeconds());
    output += ("")
    for (var i = 0; i < globalInstances.playerObjects.length; i++) {
        if (globalInstances.playerObjects[i].sessionObject != null) {
            ...
            var isPlays = false;
            var totalPlays = 0;
            for (var j = 0; j < globalInstances.playerObjects[i].sessionObject.playObjects.length; j++) {
                countPlayObjects++;
                totalPlays++;
                if (globalInstances.playerObjects[i].sessionObject.playObjects[j].title != null) {
                    output += ("       " + globalInstances.playerObjects[i].sessionObject.playObjects[j].title + " [" + globalInstances.playerObjects[i].sessionObject.playObjects[j].version + "] by: " + globalInstances.playerObjects[i].sessionObject.playObjects[j].artist + "\n\n");
...
function setSessionsRecorded() {
    db.get("SELECT sessionID FROM sessionsTable ORDER BY sessionID DESC LIMIT 1", (err, row) => {
