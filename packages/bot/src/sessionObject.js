...
class sessionObject {
  ...
  async endSession() {
    ...
    if (!this.userObjectStartOfSession) {
      globalInstances.logMessage("ERROR: no start of session info captured");
      return;
    }
    ...
    const sessionDuration = hmsToString(
      sessionDurationHours,
      sessionDurationMinutes,
      sessionDurationSeconds
    );
    ...
    for (const play of filteredPlays) {
      ...
      const songDuration = hmsToString(
        songDurationHours,
        songDurationMinutes,
        songDurationSeconds
      );
